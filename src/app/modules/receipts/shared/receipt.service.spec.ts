...
describe('ReceiptService', () => {
  ...
  describe('#listReceipts', () => {
    let expectedReceipts: Receipt[];

    beforeEach(() => {
      const items = testItems.map((t) => new Item({...t, people: testPeople}).toJson());
      expectedReceipts = testReceipts.map((r) => new Receipt({...r, list: items, people: testPeople}));
    });

    it('should return all expected receipts (called once)', () => {
      receiptService.list().subscribe(
        (receipts) => expect(receipts).toEqual(expectedReceipts, 'should return all receipts'),
        fail,
      );
    ...
    it('should return all expected results (called multiple times)', () => {
      ...
      receiptService.list().subscribe();
      receiptService.list().subscribe(
        (receipts) => expect(receipts).toEqual(expectedReceipts, 'should return expected receipts'),
        fail,
      );
  ...
  describe('#readReceipt', () => {
    ...
    it('should return a receipt given its id', () => {
      receiptService.read(expectedReceipt.id).subscribe(
        (receipt) => expect(receipt).toEqual(expectedReceipt),
        fail,
      );
